...
import { FiEdit, FiDollarSign, FiCodesandbox, FiSend, FiTrash2, FiUser, FiX } from 'react-icons/fi'
...
const NftAction: React.FC<{ nft: NftStruct }> = ({ nft }) => {
  ...
  const handlePayout = async () => {
    ...
    await toast.promise(payout(nft.id), {
  ...
  const handleMint = async () => {
    if (!address) return toast.warn('Connect wallet first')
    await toast.promise(mintNft(nft.id), {
      pending: 'Minting NFT...',
  ...
  const handleDelete = async () => {
    if (!address) return toast.warn('Connect wallet first')
    await toast.promise(deleteNft(nft.id), {
  ...
  const handleTransfer = async () => {
    ...
    await toast.promise(transferOwnership(nft.id, newOwnerAddress), {
        ...
        {isTransferModalOpen && (
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                <label htmlFor="newOwnerAddress" className="block text-sm font-medium text-gray-700 mb-1">
                  New Owner's Address
                </label>
                <div className="relative">
                  <input
                    ...
                    placeholder="0x..."
                    className="w-full p-2 pr-10 border text-black border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                  />
